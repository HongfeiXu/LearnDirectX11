...
bool InitScene()
{
	...
	Vertex verices[] =
	{
		...
		Vertex(1.0f, 1.0f, 0.8f, 1.0f, 1.0f, 0.0f, 1.0f),
		Vertex(1.0f, -1.0f, 0.8f, 1.0f, 1.0f, 0.0f, 1.0f),
		Vertex(-1.0f, -1.0f, 0.8f, 1.0f, 1.0f, 0.0f, 1.0f),
	...
	vertexBufferDesc.ByteWidth = sizeof(verices);
	...
	vertexBufferData.pSysMem = verices;
...
void DrawScene()
{
	...
	cbPerObj.WVP = XMMatrixTranspose(WVP); // When sending matrices to the effect file in direct3d11, we must send the matrix "transpose"!!!

	d3d11DevCon->UpdateSubresource(cbPerObjectBuffer, 0, 0, &cbPerObj, 0, 0); // cbPerObj structure -> applications constant buffer (cbPerObjectBuffer)
	d3d11DevCon->VSSetConstantBuffers(0, 1, &cbPerObjectBuffer); // applications constant buffer (cbPerObjectBuffer) -> Vertex Shader constant buffer
